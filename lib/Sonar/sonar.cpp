...
//   digitalWrite(DISTANCE_SENSOR_TRIG_PIN,LOW);
//   float duration= pulseIn(DISTANCE_SENSOR_ECHO_PIN,HIGH);
//   float distance= duration*0.034/2; 
//   return avg.next(distance);
// }
float lastSonarValue = 0.0f;
...
static float getFrontObstacleDistance_cm()
{
  ...
  {
    static MovingAverage movingAverage_sensor(3);
    ...
    estimated_distance = movingAverage_sensor.next(measured_distance);
